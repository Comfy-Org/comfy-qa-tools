...
def test_saying_it_not_at_all_names_every_way_of_saying_it():
    """A name, an OS, a card, or `os/card` — the four shapes `resolve` accepts.

    It used to name a fifth, `--os`/`--gpu`, which no command takes any more.
    """
    result = _run("stamp")

    assert result.exit_code != 0
    assert "os/card" in result.output
