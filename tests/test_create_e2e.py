...
def test_a_zone_google_names_in_a_stockout_jumps_the_queue(cli):
    ...
    assert result.exit_code == 0
    assert "Google suggests us-central1-c" in result.stdout
...
def test_the_number_of_attempts_is_capped_however_many_zones_google_suggests(cli):
    ...
    """
    # `spare3-a` and not `nowhere3-1-a`: `suggested_zones` only repeats things
    # shaped like a zone — a region ending in a digit, then one letter — and a
    # fixture Google would never print matches nothing, which reads as a pass.
    spares = [f"spare{n}-a" for n in range(1, 40)]
    ...
    assert "stopping after 6 zones" in result.stdout
...
def test_the_first_zone_stocks_out_and_the_second_one_takes_it(cli):
    ...
    assert "europe-west4-a has no L4 free right now" in result.stdout
...
def test_a_region_with_no_quota_is_refused_rather_than_widened(cli):
    ...
    assert "comfy-qat auth quota request --gpu l4 --region me-west1" in result.output
...
def test_the_cache_never_reaches_the_create_decision_on_its_own(cli, tmp_path):
    ...
    (tmp_path / zones_module.CACHE_NAME).write_text(
        json.dumps({"at": 1e18, "regions": {"europe-west9": 1.0, "us-east1": 2.0}}),
        encoding="utf-8")
    result = cli("--os", "linux", "--gpu", "l4", "--dry-run")
    order = order_of(result)
