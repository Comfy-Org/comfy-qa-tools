...
# a lowercase letter, then up to 62 more of letters, digits and hyphens, not
# ending in one. `_clean` turns anything into hyphens and lowercase, which is
# enough for `My Box` and not enough for `9lives` or `box_`. Checking here rather
# than letting the create fail costs nothing; letting it fail costs the quota
# read, the zone ranking and a minute of gcloud, and answers with Google's
# wording about a regular expression rather than with what to type instead.
    ...
    if preferred:
        ...
        if not _GCE_NAME.match(base):
            raise LifecycleError(
                ...
                f"starts with a letter, then letters, digits or hyphens, up to 63 "
                f"characters, and does not end in a hyphen. Nothing was created.",
...
class QuotaCheck:
    ...
    # this tool refuses.
    key: str = ""
    # Cards, not boxes. One running a3-highgpu-8g holds eight of the ceiling, and
    # counting boxes lets a create through the gate that Google then refuses.
    held: int = 0
    ...
    def problem(self) -> LifecycleError | None:
        ...
        if not self.card_limit:
            return LifecycleError(
                ...
                fix=f"comfy-qat auth quota request --gpu {self.typed} "
        ...
        if self.running and self.global_limit is not None and 0 <= self.global_limit < (
            ...
            if len(self.running) == 1:
                return LifecycleError(
                    ...
                    fix=f"comfy-qat host down {self.running[0]} — stop the one you are "
            ...
            return LifecycleError(
                ...
                fix=f"comfy-qat host down {self.running[0]} — stop the ones you are not "
...
def _cards_running(instances: list[dict]) -> int:
    ...
    before anything bills. `acceleratorCount` is in the live payload for a
    built-in G2 card as well as an attached N1 one; a row without it is counted
    as one rather than dropped, because a card that is there is spending.
    """
    held = 0
    for instance in instances or []:
        if instance.get("status") != "RUNNING":
        ...
        for accel in instance.get("guestAccelerators") or []:
            try:
                held += int(accel.get("acceleratorCount"))
...
def build(
    ...
    *, limit: int = MAX_ATTEMPTS,
    ...
    `limit` is the same cap `zones.choose` applies to the ranked list, applied
    again here because the suggestions are not on that list. `choose` hands over
    six zones and each refusal can add one more, so a chain of suggestions is a
    fall-through with no end and a command that looks hung — which is the exact
    thing the cap was written for.
    """
    ...
    if region:
        ...
        if not regions:
            raise LifecycleError(
                ...
                fix=(f"comfy-qat auth quota request --gpu {blueprint.card.key} "
