...
**`It could not be written to ~/.config/comfy-qa-tools/hosts.toml: ... Add it by hand, or run `comfy-qat host discover` to adopt it.`**
...
`comfy-qat host down` cannot reach it**. The raw `gcloud ... stop` is the only
...
`comfy-qat host discover` is the other way out, and is second because it leaves
the box running.
